...
- `generate` precomputes commit subjects, changed files, diffstat, and a bounded patch excerpt outside Codex, then passes that review context into generic `codex exec` with `--output-schema`.
