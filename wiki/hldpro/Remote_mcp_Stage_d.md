# Remote mcp Stage d

> 35 nodes · cohesion 0.11

## Key Concepts

- **stage_d_smoke.py** (19 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **run_stage_d()** (10 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **live_health_monitor.py** (9 connections) — `hldpro-governance/scripts/remote-mcp/live_health_monitor.py`
- **main()** (7 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **ProofResult** (7 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **_expect()** (6 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **main()** (5 connections) — `hldpro-governance/scripts/remote-mcp/live_health_monitor.py`
- **run_fixture()** (5 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **test_stage_d_smoke.py** (5 connections) — `scripts/remote-mcp/tests/test_stage_d_smoke.py`
- **_run_fixture()** (4 connections) — `hldpro-governance/scripts/remote-mcp/live_health_monitor.py`
- **_run_live()** (4 connections) — `hldpro-governance/scripts/remote-mcp/live_health_monitor.py`
- **_run_stdio_proof()** (4 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **_verify_audit_tamper_fails()** (4 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **_verify_audit_valid()** (4 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **test_live_health_monitor.py** (4 connections) — `scripts/remote-mcp/tests/test_live_health_monitor.py`
- **_scan_evidence_dir()** (3 connections) — `hldpro-governance/scripts/remote-mcp/live_health_monitor.py`
- **_stage_d_args()** (3 connections) — `hldpro-governance/scripts/remote-mcp/live_health_monitor.py`
- **_request()** (3 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **StageDConfig** (3 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **build_parser()** (2 connections) — `hldpro-governance/scripts/remote-mcp/live_health_monitor.py`
- **_env_has_live_markers()** (2 connections) — `hldpro-governance/scripts/remote-mcp/live_health_monitor.py`
- **_build_fixture_server()** (2 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **build_parser()** (2 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **from_env()** (2 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- **.to_dict()** (2 connections) — `scripts/remote-mcp/stage_d_smoke.py`
- *... and 10 more nodes in this community*

## Relationships

- No strong cross-community connections detected

## Source Files

- `hldpro-governance/scripts/remote-mcp/live_health_monitor.py`
- `scripts/remote-mcp/stage_d_smoke.py`
- `scripts/remote-mcp/tests/test_live_health_monitor.py`
- `scripts/remote-mcp/tests/test_stage_d_smoke.py`

## Audit Trail

- EXTRACTED: 70 (52%)
- INFERRED: 64 (48%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [[index]] to navigate.*