# Overlord Verify governance consumer

> 40 nodes · cohesion 0.11

## Key Concepts

- **verify_governance_consumer.py** (40 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_validate_record()** (20 connections) — `scripts/overlord/verify_governance_consumer.py`
- **verify()** (9 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_fail()** (6 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_load_consumer_record()** (6 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_forbidden_override_failures()** (4 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_git_root()** (4 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_is_relative_to()** (4 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_load_json()** (4 connections) — `scripts/overlord/verify_governance_consumer.py`
- **main()** (4 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_managed_file_content_failures()** (4 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_manifest_profiles()** (4 connections) — `scripts/overlord/verify_governance_consumer.py`
- **ConsumerVerifyError** (3 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_is_v2_record()** (3 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_known_profiles()** (3 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_override_records()** (3 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_profile_required_constraints()** (3 connections) — `scripts/overlord/verify_governance_consumer.py`
- **build_parser()** (2 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_consumer_record_relpath()** (2 connections) — `scripts/overlord/verify_governance_consumer.py`
- **ConsumerRecord** (2 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_contains_negated_forbidden_action()** (2 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_expected_checksum()** (2 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_expected_managed_paths()** (2 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_initial_package_version()** (2 connections) — `scripts/overlord/verify_governance_consumer.py`
- **_managed_file_entry()** (2 connections) — `scripts/overlord/verify_governance_consumer.py`
- *... and 15 more nodes in this community*

## Relationships

- No strong cross-community connections detected

## Source Files

- `scripts/overlord/verify_governance_consumer.py`

## Audit Trail

- EXTRACTED: 80 (47%)
- INFERRED: 90 (53%)
- AMBIGUOUS: 0 (0%)

---

*Part of the graphify knowledge wiki. See [[index]] to navigate.*