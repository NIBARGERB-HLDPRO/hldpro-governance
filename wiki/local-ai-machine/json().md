# json()

> God node · 17 connections · `/Users/bennibarger/Developer/HLDPRO/_worktrees/lam-graphify-phase6/supabase/functions/slack_webhook/index.ts`

## Connections by Relation

### calls
- [[handleSlackWebhook()]] `INFERRED`
- [[proxyMcp()]] `INFERRED`
- [[callMcpRouteDecision()]] `INFERRED`
- [[proxyJson()]] `INFERRED`
- [[updateInteractiveMessage()]] `INFERRED`
- [[mcpReadCircuitState()]] `INFERRED`
- [[proxyStatus()]] `INFERRED`
- [[insertStepCapabilityIssuance()]] `INFERRED`
- [[insertIntentMandate()]] `INFERRED`
- [[addReceivedReaction()]] `INFERRED`
- [[resolveCanonicalThreadTs()]] `INFERRED`
- [[fetchHostCommandById()]] `INFERRED`
- [[fetchPendingActionById()]] `INFERRED`
- [[applyPendingActionDecision()]] `INFERRED`
- [[resolveChannelRoute()]] `INFERRED`
- [[isBridgeHealthy()]] `INFERRED`

### contains
- [[index.ts]] `EXTRACTED`

---

*Part of the graphify knowledge wiki. See [[index]] to navigate.*