...
| Qwen-Coder micro-worker | `mlx-community/Qwen2.5-Coder-7B-Instruct-4bit` | on demand | 6 GB |
...
| Shadow-Critic-LAM | `mlx-community/gemma-4-26b-a4b-4bit` | on demand | 18 GB |

Operating rule: keep guardrail resident, keep MCP intent warm when memory allows, and run only one large on-demand worker/critic model at a time. Unload on completion or memory pressure. Gemma is A/B shadow-only and never satisfies authoritative QA or gate requirements.
...
- Reports Windows Ollama metadata as deprecated/off-ladder if probed.
...
- Clean implementation work may use Qwen local workers only through an approved issue-backed execution scope and downstream QA/gate evidence.
- Windows Ollama is not an active governance waterfall fallback.
