...
| GOV-034 | SQL_SCHEMA_DRIFT_PROBES | Repo-local SQL schema drift probe contract for destructive maintenance surfaces | COMPLETE | REQUIRED |
...
| GOV-034 | `docs/runbooks/sql-schema-drift-probes.md` defines the repo-local SQL/schema drift probe contract. `scripts/overlord/validate_sql_schema_probe_contract.py` validates metadata-query placement, destructive-operation preflight requirements, fixtures, stale-column negative controls, Local CI profile hooks, and residual-risk deferrals. |
...
| GOV-019 | `scripts/overlord/assert_execution_scope.py` now treats `planning_only` scopes as planning-artifact-only authority: they may validate bounded planning files, but they fail closed if they attempt to authorize implementation-shaped governance paths even when those paths were mistakenly listed in `allowed_write_paths`. |
...
| Feature ID | Notes |
|---|---|
| GOV-033 | Consumer verifier-managed session-contract surfaces can now be represented in managed consumer records as tracked session contracts (for example `CODEX.md`) and hook-settings contracts (for example `.claude/settings.json` matcher expectations), preserving stricter repo-local policies while making session-governance drift auditable. |
