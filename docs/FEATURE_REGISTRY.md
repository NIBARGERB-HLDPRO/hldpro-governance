...
| GOV-015 | The fail-fast retrieval slice now uses a repo-local governance graph when the scenario repo is governance itself, applies graph-aware ranking with one-hop link propagation, and scores relevance using both owning-file hits and expected evidence terms. The validated guidance is to pair symptom terms with mechanism/owner terms, then use bounded repo search only on the graph-returned files. |
