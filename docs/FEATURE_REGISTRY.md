...
| GOV-027 | `scripts/codex-review-template.sh` now has a tracked default reviewer persona at `docs/agents/codex-reviewer.md`, while preserving `CODEX_REVIEW_PERSONA` for operator/test overrides. Fake-Codex tests prove both persona paths reach `codex-fire.sh`. |
