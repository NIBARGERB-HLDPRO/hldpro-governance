...
| GOV-028 | REMOTE_MCP_BRIDGE_GOVERNANCE | Remote MCP Bridge standards, thin client contract, audit verifier, and operator runbook | IN_PROGRESS | REQUIRED |
...
| GOV-028 | Issue #109 Stage A adds Remote MCP Bridge invariants 11-15, a thin `scripts/som-client/` operator client, `scripts/remote-mcp/verify_audit.py`, `check-remote-mcp-audit-schema.yml`, and the `docs/runbooks/remote-mcp-bridge.md` operator runbook. Downstream HTTP/tunnel implementation and remote-machine smoke remain Stage B-D. |
