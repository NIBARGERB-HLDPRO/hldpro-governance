...
| GOV-028 | Issue #109 completed Remote MCP Bridge invariants 11-15, the thin `scripts/som-client/` operator client, `scripts/remote-mcp/verify_audit.py`, `scripts/remote-mcp/stage_d_smoke.py`, `check-remote-mcp-audit-schema.yml`, and the `docs/runbooks/remote-mcp-bridge.md` operator runbook. Issue #372 adds `scripts/remote-mcp/live_health_monitor.py`, `.github/workflows/remote-mcp-live-health.yml`, and `launchd/com.hldpro.remote-mcp-monitor.plist` for recurring health/audit monitoring. Issue #374 adds `scripts/remote-mcp/monitor_alert.py` and first fixture-run alert evidence for payload-safe operator summaries. Issue #376 selects local `launchd` as the live-authoritative operating mode while keeping GitHub Actions as the scheduled fixture harness and optional configured-live runner. |
