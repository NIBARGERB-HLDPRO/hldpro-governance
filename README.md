...
Key workflows in [`.github/workflows/`](.github/workflows/):
...
- **`overlord-sweep.yml`** — Weekly scheduled audit. Uses `docs/governed_repos.json` subsystem flags for sweep, metrics, Codex, compendium, graphify, and memory selection; explicit checkout steps are validator-backed where GitHub Actions requires static `actions/checkout` blocks.
